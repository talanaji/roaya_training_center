		...
		if($date_startmo == $mo)
			 {
		  	 if($this->session->userdata('User_type') == 1)
		{
        $sum24 =  $time_end - $time_start1;
		$sum24 =  $sum24 / 3600;
		$sum_amont55 = $dd->res_paid_price - $dd->res_teacher_percent;
		$sr5 = $sum24 *  $sum_amont55;
		$ssr5 = $ssr5 +$sr5;
         ...
         $data['new_std'] = $this->db->query("select * from students where st_active= 0 ")->result();
 		    $this->load->view('cp/templates/header',$data);
			$this->load->view('cp/welcome_message',$data);
